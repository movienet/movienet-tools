...
def timecode_to_frames(timecode, framerate):
    return int(int(timecode.split(',')[1]) * 0.001 * framerate) + sum(
        f * int(t)
        for f, t in zip((3600 * framerate, 60 * framerate, framerate),
                        timecode.split(',')[0].split(':')))


def frames_to_timecode(frames, framerate):
    ms = '{0:.3f}'.format((frames % framerate) / framerate).split('.')[1]
    return '{0:02d}:{1:02d}:{2:02d},{3:s}'.format(
        int(frames / (3600 * framerate)), int(frames / (60 * framerate) % 60),
        int(frames / framerate % 60), ms)
