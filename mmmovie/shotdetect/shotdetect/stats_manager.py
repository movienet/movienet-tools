...
class FrameMetricRegistered(Exception):
    """Raised when attempting to register a frame metric key which has already
    been registered."""

    def __init__(self,
                 metric_key,
                 message='Attempted to re-register frame metric key.'):
        # type: (str, str)
...
class FrameMetricNotRegistered(Exception):
    ...
    def __init__(
        self,
        metric_key,
        message='Attempted to get/set frame metrics for unregistered metric \
            key.'):
        # type: (str, str)
        # Pass message string to base Exception class.
...
class StatsFileCorrupt(Exception):
    """Raised when frame metrics/stats could not be loaded from a provided CSV
    file."""

    def __init__(self,
                 message='Could not load frame metric data data from passed \
            CSV file.'):
        # type: (str, str)
        # Pass message string to base Exception class.
...
class StatsFileFramerateMismatch(Exception):
    """Raised when attempting to load a CSV file with a framerate that differs
    from the current base timecode / VideoManager."""

    def __init__(self,
                 base_timecode_fps,
                 stats_file_fps,
                 message='Framerate differs between stats file and base \
                          timecode.'):
        # Pass message string to base Exception class.
...
class NoMetricsRegistered(Exception):
    """Raised when attempting to save a CSV file via save_to_csv(...) without
    any frame metrics having been registered (i.e. no shotDetector objects were
    added to the owning shotManager object, if any)."""
    pass


class NoMetricsSet(Exception):
    """Raised if no frame metrics have been set via set_metrics(...) when
    attempting to save the stats to a CSV file via save_to_csv(...).

    This may also indicate that detect_shots(...) was not called on the
    owning shotManager object, if any.
    """
    pass


class StatsManager(object):
    """Provides a key-value store for frame metrics/calculations which can be
    used as a cache to speed up subsequent calls to a shotManager's
    detect_shots(...) method. The statistics can be saved to a CSV file, and
    loaded from disk.

    Analyzing a statistics CSV file is also very useful for finding the
    optimal algorithm parameters for certain detection methods.
    Additionally, the data may be plotted by a graphing module (e.g.
    matplotlib) by obtaining the metric of interest for a series of
    frames by iteratively calling get_metrics(), after having called the
    detect_shots(...) method on the shotManager object which owns the
    given StatsManager instance.
    """

    def __init__(self):
        # Frame metrics is a dict of frame (int):
        ...
        # Flag indicating if metrics require saving.
    ...
    def register_metrics(self, metric_keys):
        ...
        Raises:
            FrameMetricRegistered: A particular metric_key has already been
                registered/added to the StatsManager.
                Only if the StatsManager is being used for read-only
                access (i.e. all frames in the video have already been
                processed for the given
    ...
    def get_metrics(self, frame_number, metric_keys):
        """Get Metrics: Returns the requested statistics/metrics for a given
        frame.
        ...
        Returns:
            A list containing the requested frame metrics for the given frame
            number in the same order as the input list of metric keys.
            If a metric could
    ...
    def set_metrics(self, frame_number, metric_kv_dict):
        ...
        Arguments:
            frame_number (int): Frame number to retrieve metrics for.
            metric_kv_dict (Dict[str, metric]): A dict mapping metric keys to
                the respective integer/floating-point metric values to set.
        """
        for metric_key in metric_kv_dict:
            self._set_metric(frame_number, metric_key,
                             metric_kv_dict[metric_key])

    def metrics_exist(self, frame_number, metric_keys):
        """Metrics Exist: Checks if the given metrics/stats exist for the given
        frame.

        Returns:
            bool: True if the given metric keys exist for the frame,
                False otherwise.
        """
        return all([
    ...
    def is_save_required(self):
        """Is Save Required: Checks if the stats have been updated since
        loading.

        Returns:
            bool: True if there are frame metrics/statistics not yet written
            to disk, False otherwise.
        """
        return self._metrics_updated

    def save_to_csv(self, csv_file, base_timecode, force_save=True):
        """Save To CSV: Saves all frame metrics stored in the StatsManager to a
        CSV file.

        Arguments:
            csv_file: A file handle opened in write mode
                (e.g. open('...', 'w')).
            base_timecode: The base_timecode obtained from the frame source
                VideoManager. If using an OpenCV VideoCapture,
                create one using the video framerate by
                setting base_timecode=FrameTimecode(0, fps=video_framerate).
            force_save: If True, forcably writes metrics out even if there are
                no registered metrics or frame statistics.
                If False, a NoMetricsRegistered will be thrown if there are
                no registered metrics, and a NoMetricsSet
        ...
        Raises:
            ...
            NoMetricsSet: No frame metrics have been entered/updated, thus
                there is no frame data to save.
        """
        csv_writer = get_csv_writer(csv_file)
        # Ensure we need to write to the file, and that we have data
        # to do so with.
    ...
    def load_from_csv(self,
                      csv_file,
                      base_timecode=None,
                      reset_save_required=True):
        """Load From CSV: Loads all metrics stored in a CSV file into the
        StatsManager instance.

        Arguments:
            csv_file: A file handle opened in read mode
                (e.g. open('...', 'r')).
            base_timecode: The base_timecode obtained from the frame source
                VideoManager. If using an OpenCV VideoCapture,
                create one using the video framerate by
                setting base_timecode=FrameTimecode(0, fps=video_framerate).
                If base_timecode is not set (i.e. is None), the framerate is
                not validated.
            reset_save_required: If True, clears the flag indicating that
                a save is required.

        Returns:
            int or None: Number of frames/rows read from the CSV file, or None
                if the input file was blank.

        Raises:
            StatsFileCorrupt: Stats file is corrupt and can't be loaded,
                or wrong file was specified.
            StatsFileFramerateMismatch: Framerate does not match the loaded
                stats file, indicating either the wrong video or wrong stats
                file was specified.
        """
        ...
        try:
            ...
            # If the file is blank or we couldn't decode anything, assume the
            # file was empty.
            return num_frames
        # First Row (FPS = [...]) and ensure framerate equals base_timecode
        # if set.
        ...
        if not row or not len(row) >= 2:
            raise StatsFileCorrupt()
        if row[0] != COLUMN_NAME_FRAME_NUMBER or row[1] != \
