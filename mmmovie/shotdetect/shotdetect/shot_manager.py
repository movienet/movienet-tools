...
def get_shots_from_cuts(cut_list, base_timecode, num_frames, start_frame=0):
    ...
    Arguments:
        ...
        num_frames (int or FrameTimecode): The number of frames, or FrameTimecode representing
            duration, of the video that was processed (used to generate last shot's end time).
    ...
    Returns:
        ...
        shot occupies based on the input cut_list.
    """
...
def write_shot_list(output_csv_file, shot_list, cut_list=None):
    ...
    Arguments:
        ...
        cut_list: Optional list of FrameTimecode objects denoting the cut list (i.e. the frames
            in the video that need to be split to generate individual shots). If not passed,
            the start times of each shot (besides the 0th shot) is used instead.
    """
...
class ShotManager(object):
    ...
    def get_cut_list(self, base_timecode):
        ...
        the shot list, noting that each shot is contiguous starting from the first frame
        and ending at the last frame detected.
        Returns:
            List of FrameTimecode objects denoting the points in time where a shot change
    ...
    def detect_shots(self, frame_source, end_time=None, frame_skip=0,
        ...
        Raises:
            ValueError: `frame_skip` **must** be 0 (the default) if the shotManager
