...
class FeatureExtractor(object):
    ...
    def __init__(self,
        ...
        if mode == "place":
            ...
            self.cfg["src_video_path"] = osp.join(data_root, "shot_split_video") if src_video_path is None else osp.join(data_root, src_video_path)
            self.cfg["dst_wav_path"] = osp.join(data_root, "aud_wav") if dst_wav_path is None else osp.join(data_root, dst_wav_path)
            self.cfg["dst_stft_path"] = osp.join(data_root, "aud_feat") if dst_stft_path is None else osp.join(data_root, dst_stft_path)
