...
class TestShotDetector(object):
    ...
    def test_shotdetect(self,):
        sdt = ShotDetector(print_list=True, save_keyf=True, save_keyf_txt=True, split_video=True,
                           begin_frame=0, end_frame=2000)
        sdt.shotdetect(self.video_path, self.out_dir)
        assert len(os.listdir(osp.join(osp.dirname(__file__), 'data/shot_keyf/test1'))) == 3*3
        with open(osp.join(osp.dirname(__file__), 'data/shot_txt/test1.txt'), "r") as f:
